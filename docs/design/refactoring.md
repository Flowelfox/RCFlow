...
- **Managed-agent base class** 🟡 (started) — `core/agents/base.py` now has `ManagedAgentBase` (shared `__init__` + `_push_subprocess_status`); all four agent collaborators inherit it, ACP's duplicate helper is gone, and the 10 uniform inline subprocess-status broadcasts in the relays now call the shared method. Still per-agent (recommended next): the near-identical `_end_*_session` / `_forward_to_*` / restart flows and the shared one-shot-CLI executor base for codex/opencode.
- **Pending** (recommended as a follow-up): finish the managed-agent base (end/forward/restart), unified tool-install pipeline (plain+streaming twins), session row↔object mapping helper, and moving domain logic (task state machine, PR-sync pipeline) out of route modules into services (see the `.importlinter` `ignore_imports` debt list).
