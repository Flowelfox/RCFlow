...
- **Domain logic out of route modules** ✅ — the task status-transition data (`VALID_TASK_TRANSITIONS`/`AI_FORBIDDEN_STATUSES` → `services/task_rules.py`) and the whole PR-sync pipeline (`serialize_pr`, `upsert_prs`, `persist_synced_prs`, `attach_pr_badges`, `sync_and_attach_prs` → `services/github_service.py`) moved from the api route modules into services. `core` and the routes now import them downward; both core→api entries are gone from the `.importlinter` allowlist (the `api → core → services → database` layers contract passes with only one remaining exception, `services.mcp_bridge → core.llm`, pending a shared-types leaf module).
- **Pending** (recommended as a follow-up): finish the install pipeline (OpenCode/Claude Code + OpenCode checksums), and move the shared core types (`ToolCallRequest`, `SessionStatus`) into a leaf module so `services.mcp_bridge` stops importing `core` (the last layering exception).
