...
| 2d — PromptRouter mixins → composition | 🔄 in progress | Converting each mixin to a composed collaborator that reaches shared router state / sibling behaviour via a `self._r` back-reference (removing the `# ty:ignore[unresolved-attribute]` annotations). **Done:** `ContextMixin` → `ContextBuilder` (`self._context`; 15 refs); `BackgroundTasksMixin` → `BackgroundTasks` (`self._background`; 71 refs); `ClaudeCodeAgentMixin`/`CodexAgentMixin`/`OpenCodeAgentMixin` → `ClaudeCodeAgent`/`CodexAgent`/`OpenCodeAgent` (`self._claude`/`self._codex`/`self._opencode`; 73 refs). PromptRouter delegates every public entry point of each collaborator so the still-mixin lifecycle code, the agentic loop, WS/route handlers, and tests call them unchanged. **Pending:** `SessionLifecycleMixin` (last one — its `_end_*_session`/`_build_*_extra_env`/`_terminate_active_monitors` callees are already delegated; it just needs the same self._r treatment) |
