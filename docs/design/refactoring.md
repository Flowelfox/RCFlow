...
| Codemod replacing hardcoded EdgeInsets with spacing tokens | ⏳ partial | Radius subset done: 99 `BorderRadius.circular(6\|10\|14)` literals across 46 files → `kRadiusSmall/Medium/Large` (1:1 unambiguous mapping, line-local edits, no whole-tree format). Padding/gap subset (`EdgeInsets.*`, `SizedBox` gaps) still pending — its value→token mapping is context-dependent (8.0 maps to kSpace2 / kPadCompact / kGapTight), so it needs human judgement per site rather than a blind sweep |
