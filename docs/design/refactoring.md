...
| Codemod replacing hardcoded EdgeInsets with spacing tokens | ✅ done (clean subsets) | Three safely-automatable, EdgeInsets-exclusive subsets codemodded to `kSpace*`/`kRadius*` tokens (line-local edits, no whole-tree format): `BorderRadius.circular(6\|10\|14)` → `kRadiusSmall/Medium/Large` (99 hits / 46 files); `EdgeInsets.all(4\|8\|12\|16\|24)` and `EdgeInsets.symmetric` `horizontal:`/`vertical:` token values → `kSpace1..5` (71 files). Base `kSpaceN` tokens used (not the semantic aliases) so each value maps 1:1 with no intent-guessing. **Deliberately left for per-site human judgement:** `EdgeInsets.fromLTRB` (mixes token + non-token values like `fromLTRB(10, 8, 10, 4)` — partial mapping reads worse), `EdgeInsets.only` (`left:`/`top:` shared with `Positioned`), and `SizedBox` gaps (`width:`/`height:` shared with `Container`) |
...
| Slice | Status | Notes |
|-------|--------|-------|
| Tests for the new transport / dispatcher / REST / sub-state seams | ⏳ partial | `test/state/stores_test.dart` unit-tests the new `LinearIssueStore` / `TaskStore` / `ArtifactStore` (sort order, byWorker bucketing, per-worker replace isolation, get/upsert/remove, forTask/forSession queries; client suite 479 → 485). `RestClient` is covered indirectly via the existing `FakeWebSocketService` worker-connection tests. `WebSocketTransport` has lifecycle/guard tests (`test/services/web_socket_transport_test.dart`: disconnected no-ops, connection-status emit, the post-dispose double-disconnect guard, broadcast streams); `PaneQueueState` has its own unit test. Socket-connected transport paths still need a fake-socket harness. Client suite 479 → 495 |
| Ruff `D` (docstrings) | ✅ format rules on; content deferred | The `D` family is enabled (pep257 convention). The docstring-**format** rules are enforced and clean: blank-line-after-section (D413), newline-after-last-paragraph (D209), blank-line-after-function (D202), first-word-capitalized (D403), trailing-period/terminal-punctuation (D400/D415), escape-sequence (D301), etc. — 50 auto-fixed + a couple hand-fixed. The docstring-**content** rules stay in `ignore` with rationale as the genuinely-incremental work (writing prose, not mechanical): D100-D107 (undocumented public API, ~280 sites), D205 (summary blank line), D401 (imperative mood), D417 (param descriptions). These come down per-module as code is touched |
