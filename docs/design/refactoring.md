...
- **Pending** (recommended as a follow-up): finish the install pipeline (OpenCode/Claude Code + OpenCode checksums), and moving domain logic (task state machine, PR-sync pipeline) out of route modules into services (see the `.importlinter` `ignore_imports` debt list).
