...
| 2d — PromptRouter mixins → composition | 🔄 in progress | Converting each mixin to a composed collaborator that reaches shared router state / sibling behaviour via a `self._r` back-reference (removing the `# ty:ignore[unresolved-attribute]` annotations). **Done:** `ContextMixin` → `ContextBuilder` (`self._context`; 15 refs); `BackgroundTasksMixin` → `BackgroundTasks` (`self._background`; 71 refs; PromptRouter delegates its 12 public `_fire_*`/`_ensure_session_row_in_db`/`_resolve_artifact_project` entry points so existing call sites are unchanged). **Pending:** `SessionLifecycleMixin`, `ClaudeCodeAgentMixin`, `CodexAgentMixin`, `OpenCodeAgentMixin` — these call one another, so each needs PromptRouter delegation for cross-collaborator + external (WS/route) callers |
