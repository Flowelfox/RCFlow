...
- **Incremental flush of active sessions**: A periodic background task (interval controlled by `SESSION_FLUSH_INTERVAL_SECONDS`, default 30 s, set to 0 to disable) upserts the session row and appends new buffer messages for every ACTIVE / EXECUTING / PAUSED session whose dirty flag is set or whose buffer has advanced since the last flush. This guarantees that title, `conversation_history`, metadata, token counters and chat messages survive an ungraceful crash or SIGKILL — without it, a crash before terminal-state archiving would leave the DB row missing or stale and `reload_stale_sessions` on the next startup would have nothing to restore.
...
4. **Drain** — At the turn-end boundary in each agent path (Claude Code `result` event, Codex / OpenCode turn end, LLM `_prompt_lock` release), `schedule_pending_drain(session)` creates a task that runs `_drain_one(session)`: pop the head of the queue, emit `message_dequeued{reason: "delivered"}`, then `handle_prompt()` to push `text_chunk(role=user, queued_id=...)` to the buffer and forward to the executor (or append to `conversation_history` on the LLM path). On the Claude Code branch the drain is scheduled via the stream task's `add_done_callback` rather than inline, so the drain fires only **after** the stream task has fully unwound (including its trailing `_drain_monitor_events` await). This avoids racing the in-progress reader against the `_forward_to_claude_code` write to stdin, which otherwise could let a CancelledError truncate the JSON line. Mid-turn delivery is not attempted for any agent: Claude Code's `--input-format stream-json` only consumes stdin between turns, and Codex / OpenCode close stdin after the initial prompt — writing mid-turn would leave the queued row marked "delivered" without ever spawning a reader task to consume the resulting turn's output, so queued messages always wait for the next turn boundary. A per-session guard (`_draining_sessions`) ensures **exactly one** `_drain_one` runs per session, so two schedule calls (e.g. the turn-end hook plus a concurrent enqueue) can never double-deliver the same message. `_drain_one` is a loop: while messages remain and `is_busy_for_queue()` is False it delivers *exactly the message it pops* (`pop_head` is the atomic claim), draining multi-message queues without waiting for the next turn-end hook. If `handle_prompt` raises, the drain emits a single `ERROR{code: QUEUED_MESSAGE_DELIVERY_FAILED}` to the buffer instead of failing silently.
5. **Clear-on-end** — When a session ends / is cancelled, `pending_store.clear_session(reason="session_ended")` deletes all rows and emits `message_dequeued` for each, and scheduled wakes are cancelled. (The lifecycle collaborator reaches these stores via `self._r._pending_store` / `self._r._wakeup_store` — reading them off `self` silently no-op'd.)

**Resuming a gated session** — When a session is paused on an open interactive gate (AskUserQuestion, plan-mode approval, plan-review — i.e. `begin_input_wait`), the agent's relay task is parked awaiting the gate, so `is_busy_for_queue()` keeps reporting busy. Resuming (`resume_session`, fired automatically when the user sends a normal message to a paused session, or via the resume button) therefore **releases any open gate** (`_release_pending_input_gates`: questions wake with no answer, plan gates are denied) so the relay unblocks and reaches a turn boundary, at which point the queued message drains. Without this a normal message queued against an open question gate could never drain — it only delivered after a manual pause (which already released the gate the same way). Answering a question (`question_answer` → `send_interactive_response`) is a separate path and does not go through resume.
