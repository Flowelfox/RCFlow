...
class RCFlowMacOSGUI:
    ...
    def _build_ui(self) -> None:
        p_outer = theme.PAD_OUTER
        p_group = theme.PAD_GROUP
        p_small = theme.PAD_SMALL
        ...
        top.grid(row=0, column=0, sticky="ew", padx=p_outer, pady=(p_outer, 0))
        ...
        lc.grid(row=3, column=0, sticky="nsew", padx=p_outer, pady=(p_small, p_outer))
    ...
    def _setup_tray(self) -> bool:
        ...
        self._delegate = _TrayDelegate.new()  # type: ignore[union-attr]
        self._delegate._gui = self  # type: ignore[union-attr]
    ...
    def _init_status_item(self) -> None:
        ...
        """
        try:
            # ty has no stubs for PyObjC; suppress unresolved-import diagnostics.
            from AppKit import (  # noqa: PLC0415
                NSImage,  # ty: ignore[unresolved-import]
                NSMenu,  # ty: ignore[unresolved-import]
                NSMenuItem,  # ty: ignore[unresolved-import]
                NSStatusBar,  # ty: ignore[unresolved-import]
                NSVariableStatusItemLength,  # ty: ignore[unresolved-import]
            )
        ...
        try:
            ...
            if img is not None:
                img.setSize_(NSMakeSize(18, 18))  # type: ignore[union-attr]
                img.setTemplate_(True)  # type: ignore[union-attr]
                self._status_item.button().setImage_(img)  # type: ignore[union-attr]

            if self._status_item.button().image() is None:  # type: ignore[union-attr]
                self._status_item.button().setTitle_("RC")  # type: ignore[union-attr]
    ...
    def _update_tray_status(self) -> None:
        ...
        if self._ns_status_text is not None:
            with contextlib.suppress(Exception):
                ...
                self._ns_status_text.setTitle_(text)  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
        if self._ns_toggle_item is not None:
            with contextlib.suppress(Exception):
                label = "Stop Server" if running else "Start Server"
                self._ns_toggle_item.setTitle_(label)  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]

    def _refresh_autostart_item(self) -> None:
        if self._ns_autostart_item is None:
            return
        with contextlib.suppress(Exception):
            self._ns_autostart_item.setState_(1 if _is_autostart_enabled() else 0)  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
...
try:
    ...
    class _TrayDelegate(NSObject):  # type: ignore[misc,valid-type]
        """Objective-C action target for NSMenuItem callbacks."""

        _gui: RCFlowMacOSGUI | None

        def init(self) -> _TrayDelegate:  # type: ignore[override]
            self = _objc.super(_TrayDelegate, self).init()  # ty: ignore[unresolved-attribute]
            if self is not None:
                self._gui = None
            return self

        @_objc.IBAction
        def toggleServer_(self, sender: object) -> None:  # noqa: N802  (PyObjC selector)
            if self._gui is not None:
                self._gui._toggle_server_requested = True

        @_objc.IBAction
        def openSettings_(self, sender: object) -> None:  # noqa: N802  (PyObjC selector)
            if self._gui is not None:
                self._gui._show_window_requested = True

        @_objc.IBAction
        def copyToken_(self, sender: object) -> None:  # noqa: N802  (PyObjC selector)
            if self._gui is not None:
                self._gui._copy_token_requested = True

        @_objc.IBAction
        def toggleAutostart_(self, sender: object) -> None:  # noqa: N802  (PyObjC selector)
            if self._gui is not None:
                _set_autostart(not _is_autostart_enabled())
                self._gui._refresh_autostart_item()

        @_objc.IBAction
        def quitApp_(self, sender: object) -> None:  # noqa: N802  (PyObjC selector)
            if self._gui is not None:
