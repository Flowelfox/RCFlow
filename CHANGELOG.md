...
### Added
- **New agent connection mode for Codex and OpenCode (experimental)** — both agents can now run over the open Agent Client Protocol instead of their bespoke integrations, opt-in per agent. Sessions gain richer streaming (visible reasoning, live plan/todo updates, token and cost reporting) and Codex gains interactive permission prompts it never had before. Claude Code is unaffected (Backend)
...
- **Pull Requests repo filter is remembered** — the repos you un-check in the Pull Requests repo filter now persist across client restarts (Client)
- **Reliable copy in chat** — every message now has a hover Copy button (and code blocks and tool output have their own), copying with line breaks and formatting intact (rich for editors that support it, clean newline-preserving text everywhere else). Tool output is now selectable, and right-clicking a selection no longer clears the highlight before the copy menu appears (Client)
- **Agent chip shows the proper name on restored sessions** — switching to an existing session (including one started from a PR, or after a reconnect) showed the agent chip as the raw internal name like "claude_code" instead of "ClaudeCode"; it now displays the same friendly name as everywhere else (Client)
- **Opening a session lands at the latest message instantly** — switching to a session now opens already scrolled to the newest message, instead of visibly racing from the top of the conversation down to the bottom (Client)
...
- **macOS one-line worker install no longer fails to start** — installing the worker on macOS with the `curl … | sh` command failed at the database-migration step with a "libpython3.12.dylib (no such file)" error and left a broken installation. The worker now installs and starts correctly on macOS (Backend)
